...
public class TipoDocumentoController {
	...
	public ResponseEntity<Iterable<Map<String, Object>>> listarAll() throws IOException, JSONException {
		return new ResponseEntity<>(tipoDocumentoService.listarAll(), HttpStatus.OK);
	}
	
	@GetMapping("/activos")
	public ResponseEntity<Iterable<Map<String, Object>>> listarActivos() throws IOException, JSONException {
		return new ResponseEntity<>(tipoDocumentoService.listarActivos(), HttpStatus.OK);
	}
	
	@PostMapping
	public ResponseEntity<String> guardar(@RequestBody String tipoDocumento) throws IOException, JSONException  {
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(SerializationFeature.FAIL_ON_EMPTY_BEANS, false);
		try {
			Map<String,Object> tipodocumento = tipoDocumentoService.guardar(tipoDocumento);
			return new ResponseEntity<>(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(tipodocumento), HttpStatus.OK);
