...
public interface IDocumentoDao extends CrudRepository<Documento, Long> {
	...
	@Query("FROM Documento d WHERE d.envio IN (SELECT e FROM Envio e WHERE "
			+ "(e.plazoDistribucion.id = ?1 AND e.tipoServicio.id =?2 AND e.productoId =?3 AND e.tipoClasificacionId=?4 AND e.tipoSeguridad.id = ?5 AND e.sedeId=?6 AND e.tipoEnvio.id=1) AND "
